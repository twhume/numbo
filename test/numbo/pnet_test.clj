...
(deftest pnet-activation-test
	(testing "Validates that spreading activation works as expected"
		(let [p (initialize-pnet initial-pnet)
				...
				(is (= 0.61 (get-in activated [:plus-1-1 :activation])))
				(is (= 0.61 (get-in activated [:plus-1-2 :activation])))
				(is (= 0.61 (get-in activated [:plus-1-3 :activation])))

				; Did no-one else get activated a bit less?
				(is (= #{:plus-1-4 :minus-4-1 :minus-50-1 :minus-5-1 :plus-1-3 :minus-7-1 :plus-1-2 :plus-1-9 :plus-1-5 :plus-1-6 :minus-10-1 :plus-1-8 :plus-1-1 :minus-3-1 :minus-6-1 :minus-8-1 :minus-9-1 :plus-1-7} (-set-with-activation activated 0.61)))

				; Did their neighbors (:2, :3, :4 but not :1) get activated?
				(is (= 0.21 (get-in activated [:2 :activation])))
				(is (= 0.21 (get-in activated [:3 :activation])))
				(is (= 0.21 (get-in activated [:4 :activation])))

				; Did no-one else get activated a bit less?
				(is (= #{:10 :4 :minus :7 :50 :8 :9 :49 :plus :2 :5 :3 :6} (-set-with-activation activated 0.21)))
				...
				(is (=
					...
					(count (-set-with-activation activated 0.01))
					))
)))
